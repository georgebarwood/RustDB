...
pub fn create_function( db: &DB, name: &ObjRef, source: Rc<String>, alter: bool )
{
  ...
  {
    let t = db.load_table( &ObjRef::new( "sys", "Function" ) ).unwrap();
    if alter
    { 
...
/// Gets a function from the database, and partially parse it ( parameters and return type ).
  ...
  if let Some(schema_id) = get_schema( db, &name.schema )
  {
    ...
    for  ( p, off ) in t.file.asc( db, Box::new(Zero{}) )
    {
      ...
      {   
        ...
        db.publish_function( name, rptr.clone() );
