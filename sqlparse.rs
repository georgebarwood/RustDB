...
impl <'a> Parser <'a>
{
    ...
    if self.token == Token::Id 
    {
      ...
      if self.test( Token::Colon )
      {
        ...
        b"WHILE" => 
          self.s_while(),
        _ => 
          panic!( "statement keyword expected, got '{}'", tos(id) )
        }
      }
    } else {
  ...
  fn read_char( &mut self ) -> u8
  {
    ...
    if self.source_ix >= self.source.len()
    {
      ...
      if cc == b'\n' 
      {
        ...
        self.source_line += 1; 
      } else if ( cc & 192 ) != 128 // Test allows for UTF8 continuation chars.
      {
        self.source_column += 1;
      }
      self.source_ix += 1;
      ...
      match sc
      {
        ...
        b'0' ..= b'9' =>
        {
          ...
          if fc == b'0' && cc == b'x'
          {
            ...
            while (b'0'..=b'9').contains(&cc) { cc = self.read_char(); }
        ...
        b'\'' =>
        {
          ...
          loop
          {
            if cc == 0 { self.err( "missing closing quote for string literal" ); }
            if cc == b'\''
            {
              cc = self.read_char();
              if cc != b'\'' { break; }
  ...
  fn read_data_type( &mut self ) -> DataType
  {
    ...
    match self.id_ref()
    {
      ...
      b"decimal"  =>
      {
         ...
         {
           p = self.read_int();
           if p < 1 { self.err( "Minimum precision is 1" ) }
           if p > 18 { self.err( "Maxiumum decimal precision is 18" ) }
           if self.test( Token::Comma) { q = self.read_int(); }
           if q < 0 { self.err( "Scale cannot be negative" ) }
           if q > p { self.err( "Scale cannot be greater than precision" ) }
           self.read( Token::RBra );
         }
  ...
  {
    if self.token != Token::Number { self.err( "Number expected" ); }
  ...
  {
    if self.token != Token::Id || self.cs != s { false } else { self.read_token(); true }
  }
  ...
  {
    if !self.parse_only { self.b.ilist.push( s ); }
  }

  /// Add a Data Operation (DO) to the instruction list.
  fn dop( &mut self, dop: DO )
  {
    if !self.parse_only { self.add( Inst::DataOp( Box::new( dop ) ) ); }
  }
...
*/
  ...
  fn values( &mut self, expect: usize ) -> TableExpression
  {
    ...
    loop
    {
      ...
      if v.len() != expect { self.err( "Wrong number of values" ); }
      values.push( v );
      if !self.test( Token::Comma ) && self.token != Token::LBra { break; } // The comma between multiple VALUES is optional.
    }
...
*/
  ...
  fn select_expression( &mut self, set_or_for: bool ) -> SelectExpression
  {
    ...
    let from = if self.test_id( b"FROM" ) { Some( Box::new( self.primary_table_exp() ) ) } else { None };
    ...
    if self.test_id( b"ORDER" )
    {
      ...
      {
        let exp = self.exp();
        let desc = if self.test_id( b"DESC" ) { true }
        else { self.test_id( b"ASC" ); false };
  ...
  fn s_insert( &mut self ) 
  {
    ...
    loop
    {
      ...
      if cnames.contains( &cname ) { self.err( "Duplicate column name" ); }
      cnames.push( cname );
      if self.test( Token::RBra ) { break; }
  ...
  fn create_table( &mut self ) 
  {
    ...
    loop
    {
      let cname = self.id();
      let typ = self.read_data_type();
      if ti.add( cname, typ ){ self.err( "Duplicate column name" ); }
      if self.test( Token::RBra ) { break; }
      if self.token != Token::Comma { self.err( "Comma or closing bracket expected" ); }
      self.read_token();
    }
  ...
  fn s_create( &mut self ) 
  {
    match self.id_ref()
    {
      ...
      b"SCHEMA" => 
        { let name = self.id(); self.dop( DO::CreateSchema( name ) );  }
      b"INDEX" =>
        self.create_index(),
      _ => 
  ...
  fn s_alter( &mut self ) 
  {
    match self.id_ref()
    {
      ...
      _ => 
        self.err("ALTER : TABLE,VIEW.. expected")
    }
  }
  ...
  fn s_drop( &mut self )  
  {
    ...
    {
      b"TABLE" =>
      { let tr = self.obj_ref(); self.dop( DO::DropTable( tr ) ); }
      b"VIEW" => { let vr = self.obj_ref(); self.dop( DO::DropView( vr ) ); }
      b"INDEX" => 
      {
        let ix = self.id();
        self.read_id( b"ON" );
