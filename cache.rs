...
impl<T> Cache<T>
{
  ...
  pub fn new() -> Self
  {
    Cache{ time:0, pages:HashMap::new(), readers:BTreeMap::new(), updates:BTreeMap::new() }
  }
  ...
  {
    if let Some(p) = self.pages.get( &pageid ) { p.get( time ) }
    else { Option::None }  
  }
...
impl<T> CachePage<T>
{
  ...
  {
    match self.values.range( (Included(&time), Included(&Time::MAX)) ).next()
    {
      Some((_k,v)) => Some(v),
      None => None
    }      
